...
- [ ] **Ajouter bandeau avec logos prestataires/projets** : Ils m'ont fait confiance - ajouter les logos avec les liens pour améliorer le netlinking et le EEAT ? Bandeau déroulant si nécessaire (cinq logos : PDA, FPCM, EH, Musée océanographique, Explore Nice Côte d'Azur ?) + plus tard (Audiovista, March on Mars ?). En attente des fichiers logos + liens de Mikaël ; prévoir un composant piloté par un tableau JS (comme `projectsData`/`articlesData`) pour que l'ajout des futurs logos soit trivial.
