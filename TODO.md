...
- [x] **Nom de domaine personnalisé** : `www.mikaelandraos.fr`, acheté chez OVH le 26/07/2026. Fichier `CNAME` ajouté à la racine, les 119 occurrences de l'ancienne URL GitHub Pages (`canonical`, `og:url`, JSON-LD, sitemap, robots.txt, README) réécrites vers le nouveau domaine — sans le segment `/portfolio`, qui disparaît avec le domaine personnalisé. DNS chez OVH : CNAME `www` → `mikaelandraos-dot.github.io.`, quatre A + quatre AAAA sur l'apex vers les IP GitHub Pages. Un TXT résiduel sur `www` (probablement issu d'un hébergement souscrit en parallèle) bloquait l'ajout du CNAME ; supprimé. Vérifié dans Settings → Pages : « DNS check successful », HTTPS forcé.
  - [ ] **À faire une fois le domaine stabilisé** (quelques jours de recul, pour ne pas agir avant que Google ait bien absorbé la redirection) : mettre à jour le lien sur LinkedIn et sur Malt, déclarer le nouveau domaine dans Search Console et y soumettre `sitemap.xml`, vérifier `docs/behance-kit.md` le jour où le compte Behance existera.
  - [x] **Sitemap validée dans Search Console** (29/07/2026) : « Traitement du sitemap réussi », 11 pages découvertes — exactement les 11 `<url>` de `sitemap.xml` (accueil, `/blog/`, `/faq.html`, 8 articles ; `404.html` volontairement absent).
- [x] **Favicon 96×96 pour l'affichage dans les résultats de recherche** (29/07/2026) : ajout d'`images/favicon-96.png` (généré depuis `apple-touch-icon.png`, 2,8 Ko) et de son `<link rel="icon" sizes="96x96">` sur les 12 pages — le 32×32 seul était en dessous du multiple de 48 px que Google recommande. Le favicon n'apparaîtra dans les résultats qu'après un nouveau passage de Googlebot ; sur un domaine migré il y a 3 jours, l'absence actuelle est attendue, pas un bug.
- [x] **Hero — alignement des symboles en filigrane** (29/07/2026) : le mark de l'ancre et le "&" en fond de hero avaient des décalages verticaux et des opacités différents (perçus comme mal alignés) ; recalés sur le même `-top-16` et une opacité proche (`0.06`). Ajout d'un troisième symbole, l'ornement ✳ déjà présent ailleurs dans le design system, à mi-hauteur à droite — les trois forment désormais un triangle de composition délibéré. Détail dans `CLAUDE.md`, section design system.
...
- [x] **Trouver un emploi à `images/blog-social-media-strategy.jpg`** : l'article `blog-articles/reseaux-sociaux-visibilite.html` (publié le 05/09/2026) l'utilise en couverture.
  - [x] **Visuel remplacé** (05/09/2026) : Mikaël a généré la photo macro proposée (pierre jetée dans l'eau, ondes concentriques) via Google AI Studio et l'a uploadée. Redimensionnée 900×502, recompressée à 26 Ko (LANCZOS, JPEG q82) — même nom de fichier, aucun changement de code nécessaire.
- [x] **Visuel de couverture pour `blog-articles/transparence-ia-loi-europeenne.html`** (05/09/2026) : Mikaël a généré une animation via Google Flow (papier levé vers la lumière d'une fenêtre, filigrane qui apparaît). Convertie de GIF (160 frames, 480×270, 8,6 Mo) en WebP animé (34 frames sur une boucle de 6,25 s, 360×202, 145 Ko) — un GIF brut de cette durée ne descend pas sous quelques Mo, même optimisé, faute d'outils de compression GIF disponibles dans l'environnement (`gifsicle`/`ffmpeg` absents) ; le WebP animé reste nativement lisible dans une balise `<img>`, sans JS ni lecteur vidéo. 145 Ko dépasse le repère de 50 Ko fixé pour les illustrations statiques : écart assumé, c'est la première couverture animée du site et un GIF/WebP anime plus de matière qu'une image fixe. À revoir avec Mikaël si un budget plus strict est souhaité pour de futures animations.
...
- [x] **QA responsive & refonte du menu mobile** (26/07/2026) : audit Playwright de 320 à 1440 px sur les 11 pages. Trois défauts corrigés — l'en-tête débordait de 66 px entre 768 et 832 px sans bouton de menu accessible, le fondu du bandeau de logos mangeait la moitié du premier logo sous 400 px, et une quinzaine de cibles tactiles tenaient sous 40 px. Le menu mobile passe d'une liste de liens à un panneau plein écran avec appels à l'action et coordonnées. Détail des règles dans `CLAUDE.md`, section « Responsive & menu mobile ».
  - [x] **Icônes harmonisées** (26/07/2026) : les douze cadres de l'accueil suivent désormais le gabarit de Prestations, le plus fin — pictogramme de 20 px au trait `1.5` dans un carré de 44 px bordé. Compétences perd son cadre de 48 px et son trait de 2, et garde sa révélation du mark au survol. Gabarit décrit dans `CLAUDE.md`.
