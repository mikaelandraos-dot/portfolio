...
- [x] **Trouver un emploi à `images/blog-social-media-strategy.jpg`** : l'article `blog-articles/reseaux-sociaux-visibilite.html` (publié le 05/09/2026) l'utilise en couverture.
  - ⚠️ **Le visuel lui-même reste à remplacer avant fusion** : c'est un rendu 3D « réseau de nœuds » avec le texte anglais « SOCIAL MEDIA STRATEGY » incrusté dans l'image — hors charte (esthétique tech générique proscrite par le design system, texte en anglais sur un site entièrement en français, ne suit ni la ligne encre/ligne ni la ligne photographique ivoire des 8 autres couvertures). Conservé provisoirement pour ne pas casser le build. Un prompt Google AI Studio (photo macro d'une pierre jetée dans l'eau, ondes concentriques, dans la veine de `blog-cover-design-sobriete.jpg`) a été fourni à Mikaël pour générer le remplacement.
